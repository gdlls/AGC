...
tasks.jar {
    manifest {
        ...
        val hasGitMetadata = rootProject.layout.projectDirectory.file(".git").asFile.exists()
        ...
        val gitHash = if (hasGitMetadata) git.exec(providers, "rev-parse", "--short=7", "HEAD").get().trim() else "nogit"
        val implementationVersion = "$mcVersion-${build ?: "DEV"}-$gitHash"
        val date = if (hasGitMetadata) git.exec(providers, "show", "-s", "--format=%ci", gitHash).get().trim() else buildTime.toString()
        val gitBranch = if (hasGitMetadata) git.exec(providers, "rev-parse", "--abbrev-ref", "HEAD").get().trim() else "local-zip"
        attributes(
            "Main-Class" to "org.bukkit.craftbukkit.Main",
            "Implementation-Title" to "AGC",
            "Implementation-Version" to implementationVersion,
            "Implementation-Vendor" to date,
            "Specification-Title" to "AGC",
            "Specification-Version" to project.version,
            "Specification-Vendor" to "AGC",
            "Brand-Id" to "agcmc:agc",
            "Brand-Name" to "AGC",
...
val agcPaperclipArchiveName = "agc-paperclip-${project.version}.jar"
val createAgcPaperclipJar by tasks.registering(Copy::class) {
    group = "bundling"
    description = "Copies the runnable Paperclip jar using the AGC distribution name"
    dependsOn(tasks.createPaperclipJar)
    from(tasks.createPaperclipJar.flatMap { it.outputZip })
    into(layout.buildDirectory.dir("libs"))
    rename(".*", agcPaperclipArchiveName)
}

tasks.createPaperclipJar {
    finalizedBy(createAgcPaperclipJar)
}
