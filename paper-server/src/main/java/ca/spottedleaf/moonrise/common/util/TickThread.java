...
public class TickThread extends Thread {
    ...
    // AGC - honesty note: this guard used to also return true for any thread that had entered an
    // "AgcPluginVirtualizer" virtual-primary context, which let world-tick worker threads pass the
    // chunk-system/AsyncCatcher thread guard while plugins still believed they were on the main
    // thread. That is exactly the invariant the guard exists to protect, so the virtual-primary
    // escape hatch was removed: a tick thread is a TickThread instance, nothing else.
